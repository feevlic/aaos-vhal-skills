---
name: oem-vhal-skills-template
description: Use when: implementing, debugging, or mocking OEM VHAL properties. Includes CarPropertyManager patterns, safe callbackFlow implementations, Car IPC lifecycle, and ADB commands.
---

# OEM Automotive OS Release Skill for VHAL

<system_directives>

1. **Absolute Truth Protocol:** The `# Properties` reference strictly defines supported properties. You MUST use the `properties-reference.md` file in the current folder to check the supported properties. Do not invent properties, IDs, or types. Unlisted properties are unsupported, vendor specific properties start with(`0x2...`), do not hallucinate hex values for them. It may that they have more strict privileged permission and require own Permission handling.
...
### 4. Actuation Latency & State Flickering (The "Echo" Race Condition)
...
```bash
adb root
adb shell cmd car_service inject-vhal-event 0x1120040A 0 1
```
...
```bash
adb root
adb shell cmd car_service get-property-value 0x11401020 0
```
...
Feel free to adjust this file or change it to your needs.
