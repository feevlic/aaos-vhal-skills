...
description: Use when: implementing, debugging, or mocking Android 15 VHAL properties. Includes CarPropertyManager patterns, callbackFlow strategies, and ADB terminal commands for Automotive OS.
---
...
5. **Feature-Flag Awareness:** If a property requires an Android 15 Feature Flag (e.g. `@FlaggedApi`), you MUST instruct the developer to perform a conditional check using `android.car.feature.Flags.<flag_name>()` before accessing the property.
6. **Strict Vendor Guardrails:** Many OEMs use custom properties starting with `0x2`. If the user asks for a property not in the properties reference, you MUST state that it is a vendor-specific property, and you MUST NOT hallucinate hex values, parameters, or types for it.
   </operational_constraints>
   </system_directives>
...
Consult [Properties Reference](./properties-reference.md) for the exact Property IDs, required Permissions, Configurations, and Data Types for Android 15 VHAL properties.
